...
# Especificação dos tokens e das suas expressões regulares
TOKEN_SPECIFICATION = [
    ('NUMBER',   r'\d+(\.\d*)?'),  # Inteiros ou números de ponto flutuante
    ('PLUS',     r'\+'),
    ('MINUS',    r'-'),
    ('MULTIPLY', r'\*'),
    ('DIVIDE',   r'/'),
    ('LPAREN',   r'\('),
    ('RPAREN',   r'\)'),
    ('SKIP',     r'[ \t]+'),       # Ignorar espaços e tabs
    ('MISMATCH', r'.'),            # Qualquer outro caracter (erro)
]
...
def tokenize(code_string):
    ...
    tokens.append(Token('EOF', None)) # Token de Fim de Arquivo (End Of File)
    return tokens
...
class Parser:
    ...
    def _advance(self):
        ...
        if self.pos < len(self.tokens):
            ...
            # Se self.pos == len(self.tokens), current_token já é EOF e não deve avançar mais.
            # Esta condição é mais para segurança, pois EOF deve ser o último.
            self.current_token = Token('EOF', None) 

    def _eat(self, token_type):
        """Consome o token atual se for do tipo esperado, senão levanta erro."""
        if self.current_token.type == token_type:
            token_value = self.current_token.value # Guarda o valor antes de avançar
            self._advance()
            return token_value # Retorna o valor do token consumido (útil para NUMBER)
        else:
            raise ParserError(
    ...
    # Regra: factor -> NUMBER | LPAREN expr RPAREN | MINUS factor
    def _factor(self):
        ...
        if token.type == 'NUMBER':
            ...
            self._eat('LPAREN')
            result = self._expr() # Chamada recursiva para a sub-expressão
            self._eat('RPAREN')
            return result
        elif token.type == 'MINUS': # Menos unário
            self._eat('MINUS')
            # O resultado é o negativo do fator seguinte
            # Permite -5, -(2+3), --5 (que vira 5), etc.
            return -self._factor() 
        else:
            raise ParserError(
    ...
    # Regra: term -> factor term_prime
    # term_prime é implementado iterativamente aqui para lidar com associatividade à esquerda
    def _term(self):
        node_value = self._factor() # Obtém o valor do primeiro fator

        # Implementa a parte 'term_prime' da gramática
        # term_prime -> MULTIPLY factor term_prime | DIVIDE factor term_prime | epsilon
        while self.current_token.type in ('MULTIPLY', 'DIVIDE'):
            ...
            if op_token_type == 'MULTIPLY':
                ...
                node_value /= right_factor_value # Divisão de ponto flutuante
        return node_value

    # Regra: expr -> term expr_prime
    # expr_prime é implementado iterativamente aqui
    def _expr(self):
        node_value = self._term() # Obtém o valor do primeiro termo

        # Implementa a parte 'expr_prime' da gramática
        # expr_prime -> PLUS term expr_prime | MINUS term expr_prime | epsilon
    ...
    def parse(self):
        ...
        if not self.tokens or self.tokens[0].type == 'EOF': # Verifica se a entrada está vazia
             raise ParserError("Não é possível analisar uma entrada vazia.")
        
        result = self._expr() # Começa pela regra inicial 'expr'
        
        # Após _expr() retornar, devemos estar no token EOF se a expressão estiver correta
